...
def load_variable_file():
    ...
    variables_path = os.path.join(script_directory, "9200L_variables.json")
...
def get_log_path():
    ...
    """
    script_directory = os.path.dirname(os.path.abspath(__file__))
    logfile_path = os.path.join(script_directory, "9200L_log.txt")
    return logfile_path
...
def boot_device():
    ...
    if boot_handle == 2:
        ...
        objTab.Screen.Send(end_line, 20)
...
def call_directory(directory):
    ...
    check_for_directory = objTab.Screen.WaitForStrings(['Attributes'.format(directory),
