...
def boot_device():
    ...
    :return:
    """
    # Send the boot command to the screen
    objTab.Screen.Send('{}{}'.format(variables['boot_command'], end_line))

    # Wait for either [yes/no], "Press RETURN to get started!", or the custom boot message
    boot_handle = objTab.Screen.WaitForStrings(['[yes/no]:', 'Press RETURN to get started!', variables['boot_message']])

    if boot_handle == 1:  # [yes/no] prompt
        objTab.Screen.Send('no' + end_line)
        log_message('boot_device: boot_handle == 1')

        # After sending 'no', check if the next prompt is "Press RETURN to get started!"
        boot_handle = objTab.Screen.WaitForStrings(['Press RETURN to get started!', variables['boot_message']],
                                                   timeout=10)  # timeout to prevent endless loop
        if boot_handle == 1:  # "Press RETURN to get started!" found
            objTab.Screen.Send(end_line)
            log_message('boot_device: Press RETURN to get started!')

            # After passing "Press RETURN", check again for the [yes/no] prompt
            boot_handle = objTab.Screen.WaitForStrings(['[yes/no]:', variables['boot_message']], timeout=10)
            if boot_handle == 1:  # [yes/no] prompt appears again
                objTab.Screen.Send('no' + end_line)
                log_message('boot_device: [yes/no] found again after RETURN')

    elif boot_handle == 2:  # "Press RETURN to get started!" prompt
        objTab.Screen.Send(end_line)
        log_message('boot_device: boot_handle == 2')

        # After sending the return, check if [yes/no] shows up
        boot_handle = objTab.Screen.WaitForStrings(['[yes/no]:', variables['boot_message']], timeout=10)
        if boot_handle == 1:  # [yes/no] prompt found
            objTab.Screen.Send('no' + end_line)
            log_message('boot_device: [yes/no] found after RETURN')
