...
Revised for use with C9k series Cisco switches on 7/2/24 by Mitch Kelley

cisco_9300_reset.py

The purpose of this script is to remove files on C9K devices
...
def load_variable_file():
    ...
    variables_path = os.path.join(script_directory, "9300_variables.json")
...
def get_log_path():
    ...
    """
    script_directory = os.path.dirname(os.path.abspath(__file__))
    logfile_path = os.path.join(script_directory, "9300_log.txt")
    return logfile_path
...
def boot_device():
    ...
    boot_handle = objTab.Screen.WaitForStrings(['[yes/no]:', variables['boot_message']])
    if boot_handle == 1:
        objTab.Screen.Send('no' + end_line, 20)
        log_message('boot_device: boot_handle == 1')

    if boot_handle == 2:
        # objTab.Screen.WaitForString(variables['boot_message'])
        objTab.Screen.Send(end_line)
        objTab.Screen.Send(end_line, 30)
        log_message('boot_device: boot_handle == 2')

    else:
        log_message("boot_handle: Boot messages not found!")

    change_to_boot_prompt()
    objTab.Screen.WaitForString(variables['prompt'])
    log_message('boot_device: Found Prompt,  Sending enable command!')
...
# Function originally intended to handle rommon erasure on 2960x
...
def get_directory_contents(directory):
    ...
    if directory_exists:
        ...
        for count, row in enumerate(file_rows):
            ...
            if row.strip():
                ...
                if ".conf" in row or ".pkg" in row or ".bin" in row:
                    ...
                    log_message('get_directory_contents: Found .conf/.bin file!')
                    log_message('ROW' + row)
                    continue
                ...
                if 'bytes' in row:
                    log_message('get_directory_contents: Found bytes, break!')
                    break
        ...
        if files and found_conf_file:  # If files were detected, return them as a list
            ...
            display_to_user('Couldn\'t find .conf or .bin file!')
            return None
...
def get_row_info(row, directory):
    ...
    # sym_filename = directory + '/' + short_filename + 'x'
    # if "/." in full_filename or "/.." in full_filename:
    #     log_message('get_row_info: Filename {}'.format(sym_filename))
    #     log_message("FULL_FILENAME:" + sym_filename)
    #     return sym_filename
    # else:
    #     log_message('get_row_info: Filename {}'.format(full_filename))
    #     log_message("FULL:" + full_filename)
...
def call_directory(directory):
    ...
    # check_for_directory = objTab.Screen.ReadString("switch:")
    # if "Size" in check_for_directory:
    #     return True
    #
    # if "no such file or directory" in check_for_directory:
    #     return False
    #
    # elif "(Aborted)" in check_for_directory:
    #     objTab.Screen.Send("dir {}{}".format(directory, end_line))
    # Wait for confirmation on the existence of the directory.
    check_for_directory = objTab.Screen.WaitForStrings(['Size'.format(directory),
                                                       'no such file or directory'])
    # Return a boolean with the existence of the directory.This boolean is specific to the 9300
...
# def directory_missing(directory):
#     flash_error = objTab.Screen.ReadString(variables['prompt'])
#     check_for_directory = objTab.Screen.WaitForStrings(['Size'.format(directory),
#                                                         'no such file or directory'])
#     while True
#         if "(Aborted)" in flash_error:
#             objTab.Screen.Send("dir {}{}".format(directory, end_line))
#
#         if "Size" in flash_error or variables['prompt'] in flash_error:
...
# Determine boot file
# def boot_var():   #Use boot error instead??
#     """
...
#     log_message('clear_keys: zeroize RSA sent!')
#     confirm_or_continue()
#     objTab.Screen.WaitForStrings(variables['prompt'])
#     log_message('clear_keys: Found Prompt!')


# def clear_keys_ec():
#     objTab.Screen.Send("crypto key zeroize ec" + end_line)
#     log_message('clear_keys: zeroize EC sent!')
